...
from tools.data_utils import get_mn_test_image_pair, get_sample_mn_image_pair, get_mn_image_pair_v2
...
def main_match_train(mode, config, model_dir=None):
    ...
    get_mn_image_pair_v2(video_path_head, image_path_head, video_annos_path_head, image_annos_path_head, path_head_save)
    mn_image_pair_data_list = get_sample_mn_image_pair(path_head_save, 2, 3)
    ...
    for each in mn_image_pair_data_list:
        p1=each['video_cut_img_path']
        p2=each['img_path']
        label=each['label']
    ...
    count = 0
    count_all = len(raw_video_frame_path)
    batch_size = 5
    ...
    for p1, p1_feature_path, p1_name, p2, p2_feature_path, p2_name in zip(raw_video_frame_path, video_feature_path,
        ...
        p1_path_name = p1_feature_path + "/" + p1_name
        if p1_path_name not in image_name_all:
            image_1 = load_image_gt_v2(p1, config)
            image_all.append(image_1)
            p_feature_path_all.append(p1_feature_path)
            p_name_all.append(p1_name)
            image_name_all[p1_path_name] = 1  ##.append( )
            pross_count += 1
        else:
            image_name_all[p1_path_name] += 1

        p2_path_name = p2_feature_path + "/" + p2_name
        if p2_path_name not in image_name_all:
            image_2 = load_image_gt_v2(p2, config)
            image_all.append(image_2)
            p_feature_path_all.append(p2_feature_path)
            p_name_all.append(p2_name)
            image_name_all[p2_path_name] = 1  ##.append( )
            pross_count += 1
        else:
            image_name_all[p2_path_name] += 1

        if (pross_count % batch_size == 0) or (pross_count % batch_size == 1) or (count == count_all):
            # print("count", count, len(image_1_all))
            if len(image_all) > 0:
                print("save feature:", pross_count)
                rcnn_model.save_dataset_train(image_all, p_feature_path_all, p_name_all)
            # print("--------")
            ##rcnn_model.save_dataset_train(image_2_all, p2_feature_path_all, p2_name_all)
...
def main_match_test(mode, config, model_dir=None):
    ...
    for p1 in test_video_path_list:
        ...
        for p2 in test_img_path_list:
            ...
            p2_image_name = p2.split("/")[-1]
            img_name.append(p2_image_name)
            img_feature_path.append(
                constant.path_head_save + "{}_data_feature/image_feature/".format(mode) + p2_image_path)
