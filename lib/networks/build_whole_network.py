...
from lib.networks import mobilenet_v2
from lib.box_utils import encode_and_decode
from lib.box_utils import boxes_utils
from lib.box_utils import anchor_utils
from lib.configs import cfgs
from lib.losses import losses
from lib.losses import tfapi_loss
from lib.box_utils import show_box_in_tensor
from lib.detection_oprations.proposal_opr import postprocess_rpn_proposals
from lib.detection_oprations.anchor_target_layer_without_boxweight import anchor_target_layer
from lib.detection_oprations.proposal_target_layer import proposal_target_layer
...
class DetectionNetwork(object):
        ...
        with tf.variable_scope('Fast-RCNN_{}'.format(scope)):
            ...
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                ...
                if not scope == 'stage3':
                    ...
                    cls_score = tf.reshape(cls_score, [-1, cfgs.CLASS_NUM + 1])
                    bbox_pred = tf.reshape(bbox_pred, [-1, 4 * (cfgs.CLASS_NUM + 1)])
                    return fc_flatten, bbox_pred, cls_score
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        fc_flatten_stage3, stage3_bbox_pred, stage3_cls_score = self.build_fastrcnn(feature_to_cropped=feature_to_cropped,
                                                                 rois=stage3_rois,
                                                                 img_shape=img_shape,
                                                                 scope='stage3')
        # bbox_pred shape: [-1, 4*(cls_num+1)].
        ...
        if not self.is_training:
            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                ...
                return fc_flatten_stage3, self.postprocess_fastrcnn(rois=stage3_rois, bbox_ppred=stage3_bbox_pred, scores=cls_prob, img_shape=img_shape, scope='stage3')
        else:
            '''
