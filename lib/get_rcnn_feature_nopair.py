...
#            print("image_select_shape:", image_data_select.shape)
    ...
    def save_dataset_train(self, images, data_path_all, image_name_all):
        ###print("img:", len(images) )
        ###print("img_label:", len(labels) )
        ...
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        #######print("molded_images:", len(molded_images) )
        anchors = np.broadcast_to(anchors, (self.config.BATCH_SIZE,) + anchors.shape)
        fpn_p5_all, output_rois_all, output_rois_score_all = self.output.predict([molded_images, image_metas, anchors], verbose=0)

        img_data = []
        img_fpn_data = []
        for fpn_p5, output_rois, output_rois_score, data_path, image_name in zip(fpn_p5_all, output_rois_all, output_rois_score_all, data_path_all, image_name_all):
            label_count = output_rois_score.shape[-1]
            #####print("output_rois_score",output_rois_score.shape)
            label_dict = {}
            for ii in range(1, label_count):  # 因为0是背景类
                tmp = output_rois_score[:, ii]
                ##print("tmp_shape:", tmp.shape)
                tmp_idx = np.argsort(-tmp)[0:2]  ###np.argmax(tmp)[:-1]
                tmp_score = tmp[np.argsort(-tmp)[0:2]]
                label_dict[ii] = [(tmp_score[0], tmp_idx[0]), (tmp_score[1], tmp_idx[1])]
            ...
            tmp_rois = np.zeros((4, 7, 7, 256))
            # if tmp_rois_score[0][1][0][0] > 0.5:
            tmp_rois[0] = output_rois[tmp_rois_score[0][1][0][1]]
            if tmp_rois_score[0][1][1][0] > 0.5:
                tmp_rois[1] = output_rois[tmp_rois_score[0][1][1][1]]
            ##
            if tmp_rois_score[1][1][0][0] > 0.5:
                tmp_rois[2] = output_rois[tmp_rois_score[1][1][0][1]]
            if tmp_rois_score[1][1][1][0] > 0.5:
                tmp_rois[3] = output_rois[tmp_rois_score[1][1][1][1]]
