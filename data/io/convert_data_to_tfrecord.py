...
def convert_pascal_to_tfrecord():
    save_dir = '/data/wl_data/myspace/tf_records_1/'

    for i in range(1, 7):
        xml_path = '../../../SSD/train_xml_%d/'%i
        frame_path = '/data/wl_data/myspace/train_dataset_part%d/video_cut/'%i
        image_path = '/data/wl_data/live_data/train_dataset_part%d/image/'%i
        save_path = save_dir + FLAGS.dataset + '_' + FLAGS.save_name + '_%d.tfrecord'%i
        mkdir(save_dir)
        writer = tf.python_io.TFRecordWriter(path=save_path)
        # writer_options = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.ZLIB)
        # writer = tf.python_io.TFRecordWriter(path=save_path, options=writer_options)
        for count, xml in enumerate(glob.glob(xml_path + '/*/0.xml')):
            # to avoid path error in different development platform
            xml = xml.replace('\\', '/')
            xml_split = xml.split('/')[-2:]
            if len(xml_split[0]) == 6:
                img_name = xml_split[0] + '/' + xml_split[1].split('.')[0] + FLAGS.img_format
                img_path = os.path.join(frame_path, img_name)
            else:
                img_name = xml_split[0][1:] + '/' + xml_split[1].split('.')[0] + FLAGS.img_format
            ...
            view_bar('Conversion progress', count + 1, len(glob.glob(xml_path + '/*/0.xml')))
