...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    if dataset_name not in ['ship', 'spacenet', 'pascal', 'coco', 'tb']:
    ...
    if is_training:
        pattern = os.path.join('/data/wl_data/myspace/tf_records_1', 'pascal_train*')
    else:
        pattern = os.path.join('../data/tfrecord_1', dataset_name + '_test*')
